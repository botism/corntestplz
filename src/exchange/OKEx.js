...
class OKEx extends Component {
    ...
    static getSymbolMini = (symbol) => {
        ...
        return {
            BTCUSDT: 'btc',
            BTCUSD191227: 'BTC-DEC27',
            BTCUSD191101: 'BTC-NOV01',
            BTCUSD191025: 'BTC-OCT25',
            BTCUSDSWAP: 'BTC-PERP',
            ETHUSDT: 'eth',
            ETHUSD191227: 'ETH-DEC27',
            ETHUSD191101: 'ETH-NOV01',
            ETHUSD191025: 'ETH-OCT25',
            ETHUSDSWAP: 'ETH-PERP',
    ...
    componentDidMount() {
        ...
        setInterval(() => {
            ...
            OKEx.tholder.forEach((thold) => {
                ...
                if (total > 10000000) {
                    console.debug('okex trade >10 ' + JSON.stringify(thold))
                    return
                ...
                if (thold.symbol.includes('USDT')) {
                    t[12] = (Math.abs(thold.amt) >= 10 ? Formatter.getKformat(Math.abs(thold.amt), 0) : Math.abs(thold.amt) < 0.1 ? Math.abs(thold.amt).toFixed(2) : Math.abs(thold.amt).toFixed(1)) + ' ' + OKEx.getSymbolMini(thold.symbol)
                } else {
                    t[12] = OKEx.getSymbolMini(thold.symbol)
                }
                ...
                Ticker.instances.forEach((ticker) => {
                    ...
                    // console.log('OKExOn: ' + ticker.state.OKExOn)
        ...
        const socketMessageListener = msg => {
            ...
            try {
                if (data instanceof String) {
                    ...
                    json = JSON.parse(pako.inflateRaw(data, {to: 'string'}))
                    // console.log("json",json);
                }
            } catch (error) {
                console.log(error)
            ...
            let t = {
                symbol: tradeRaw.instrument_id.replace('-','').replace('-',''),
                price: pp,
        ...
        const socketOpenListener = e => {
            ...
            this.state.ws.send("{\"op\": \"subscribe\", \"args\": [\"spot\/trade:BTC-USDT\"]}")

            this.state.ws.send("{\"op\": \"subscribe\", \"args\": [\"futures\/trade:BTC-USD-191227\"]}")
            this.state.ws.send("{\"op\": \"subscribe\", \"args\": [\"futures\/trade:BTC-USD-191101\"]}")
            this.state.ws.send("{\"op\": \"subscribe\", \"args\": [\"futures\/trade:BTC-USD-191025\"]}")
            //
            this.state.ws.send("{\"op\": \"subscribe\", \"args\": [\"swap\/trade:BTC-USD-SWAP\"]}")
            //
            //
            this.state.ws.send("{\"op\": \"subscribe\", \"args\": [\"spot\/trade:ETH-USDT\"]}")

            this.state.ws.send("{\"op\": \"subscribe\", \"args\": [\"futures\/trade:BTC-USD-191227\"]}")
            this.state.ws.send("{\"op\": \"subscribe\", \"args\": [\"futures\/trade:BTC-USD-191101\"]}")
            this.state.ws.send("{\"op\": \"subscribe\", \"args\": [\"futures\/trade:BTC-USD-191025\"]}")

            this.state.ws.send("{\"op\": \"subscribe\", \"args\": [\"swap\/trade:ETH-USD-SWAP\"]}")
            ...
            <div id={'exchangeDiv'}>
                ...
                <div id={'exchangeDetails'}
                    ...
                    <div style={{display: 'flex', alignItems: 'center'}}>
                        ...
                        <img src={'images/exchange/OKEx.png'} id={'exchangeIconn'} alt={'OKEx'}
                             onMouseEnter={() => {
                        ...
                        <span id={'tittleee'} className={'hoverrActive'} style={{transition: '0.5s'}}>OKEx</span>
                        ...
                        <div style={{padding: 4, paddingLeft: 12}}>
                            <Toggle
                                ...
                                onChange={(e) => {
                                    ...
                                    Prices.savee.OKExOn = e.target.checked
                ...
                <img src={'images/exchange/OKEx.png'} id={'bitmexBottomIcon'} alt={'OKEx'} onClick={() => {
...
export default onClickOutside(OKEx)
