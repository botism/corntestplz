...
export class Bybit extends Component {
    ...
    componentDidMount() {
        ...
        setInterval(()=>{
            ...
            Bybit.tholder.forEach((thold)=> {
                ...
                // t[12] = thold.symbol==='XBTUSD'?'':Bybit.getSymbolMini(thold.symbol)
                    // t[12] = (Math.abs(thold.amt) >= 1? Formatter.getKformat(Math.abs(thold.amt), 0):Math.abs(thold.amt)<0.1?Math.abs(thold.amt).toFixed(2):Math.abs(thold.amt).toFixed(1)) + ' ' + Bybit.getSymbolMini(thold.symbol)

                // (Math.abs(thold.amt) >= 1? Formatter.getKformat(Math.abs(thold.amt), 0):Math.abs(thold.amt).toFixed(2))
                    // + ' '
                    //     Bybit.getSymbolMini(thold.symbol)
                // }
        ...
        const socketMessageListener = msg => {
            ...
            if (trade == null || !trade.data) {
                return
            }
            // console.log('msg: ' + JSON.stringify(trade))
            ...
            let trade_ = trade.data[0]

            let symbol = trade_.symbol
            ...
            let t = {
                ...
                timestamp: trade_.cross_seq,
                side: trade_.side==='Buy',
                exchangeName: "bybit",
                startTime: Date.now(),
                amt: trade_.size,
                firstPrice: trade_.price,
                lastPrice: trade_.price
            }
            ...
            Bybit.tholder.forEach((thold) => {
                // console.log(thold.timestamp + ' ::: ' + t.timestamp + ' : ' + (thold.timestamp === t.timestamp))
                if (thold.timestamp === t.timestamp && thold.symbol===t.symbol && thold.side === t.side) {
                    ...
                    updated = true
                    // console.log('bybit update to ' + thold.size)
                }
            })

            if (!updated) Bybit.tholder.push(t)
        ...
        const socketCloseListener = e => {
            ...
            setTimeout(()=>{
                ...
                if (Prices.savee.bybitOn === true) {

                    this.setState({ws: new WebSocket('wss://stream.bybit.com/realtime')})
            ...
            <div id={'exchangeDiv'}>
                ...
                <div id={'exchangeDetails'}
                    ...
                    <div style={{display: 'flex', alignItems: 'center'}}>
                        ...
                        <img src={'images/exchange/bybit.png'} id={'exchangeIconn'} alt={'bybit'}
                             onMouseEnter={() => {
                        ...
                        <span id={'tittleee'} className={'hoverrActive'} style={{transition: '0.5s'}}>bybit</span>
                        ...
                        <div style={{padding: 4, paddingLeft: 12}}>
                            <Toggle
                                ...
                                onChange={(e) => {
                                    ...
                                    Prices.savee.bybitOn = e.target.checked
                ...
                <img src={'images/exchange/bybit.png'} id={'bitmexBottomIcon'} alt={'bybit'} onClick={() => {
...
export default onClickOutside(Bybit)
